...
from .vgg_arch import VGGFeatureExtractor,VGGFeatureExtractorFundus
from ..arch import SwinTransformer
...
class FundusPerceptualLoss(nn.Module):
    """Perceptual loss with commonly used style loss. USing the weight of fundus classification pretrained weight
    ...
    def __init__(self,
                 ...
                 style_weight=0.,
                 criterion='l1'):
        super(FundusPerceptualLoss, self).__init__()
        self.perceptual_weight = perceptual_weight
        self.style_weight = style_weight
        self.layer_weights = layer_weights
        self.vgg = VGGFeatureExtractorFundus(
            layer_name_list=list(layer_weights.keys()),
            vgg_type=vgg_type,
            use_input_norm=use_input_norm,
            range_norm=range_norm)
...
class FundusClassLoss(nn.Module):
    """FundusClassLoss using SWIN slim weight.
    ...
    def __init__(self,resize=True,criterion='ce',loss_weight=1.0):
        super(FundusClassLoss, self).__init__()
        self.loss_net = SwinTransformer(img_size=256,
                                        patch_size=4,
                                        in_chans=3,
                                        num_classes= 4 ,
                                        embed_dim= 96,
                                        ...
                                        sigmoid= True,)
        param_key = torch.load("/data/huden/CATINTELL/pretrained_networks/SWIN_loss_Net.pth")
        
        # for k in ["total_ops", "total_params", "features.total_ops", "features.total_params", "classifier.total_ops", "classifier.total_params"]:
        #     param_key['params'].pop(k)
        # vgg_net.load_state_dict(state_dict['params'])
        self.loss_net.load_state_dict(param_key['params'],strict= False)
        device="cuda" if torch.cuda.is_available() else "cpu"
        self.loss_net.to(device=device)
        self.loss_net.eval()
        self.loss_weight=loss_weight
        self.resize=resize
    ...
    def forward(self, x, gt):
        ...
        # extract vgg features
        if self.resize:
            resi=torchvision.transforms.Resize((256, 256))
            x=resi(x)
        x_features = self.loss_net(x)
        # print(x_features)

        return self.criterion(x_features,gt)*self.loss_weight
